...
echo '    LA_SPARKMETRIC_REGEX=app.*\.ExternalShuffle\.shuffle-client\.usedDirectMemory|app.*\.jvm\.pools\.PS-Eden-Space\.used|app.*\.jvm\.pools\.PS-Old-Gen\.used|app.*\.jvm\.total\.used|app.*\.jvm\.pools\.PS-Survivor-Space\.used|app.*\.jvm\.pools\.PS-Old-Gen\.used-after-gc|app.*\.jvm\.pools\.Code-Cache\.used|app.*\.jvm\.pools\.Metaspace\.used|app.*\.executor\.cpuTime|app.*\.executor\.RunTime    '
